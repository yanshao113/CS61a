...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        branch_lft = totals_tree(end(left(m)))
        branch_rht = totals_tree(end(right(m)))
        return tree(total_weight(m),[branch_lft,branch_rht])
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    for branch in branches(t):
        new_branches = new_branches + [replace_loki_at_leaf(branch, lokis_replacement)]
...
def sub_interval(x, y):
    ...
    and any value in y."""
    "*** YOUR CODE HERE ***"
    sb_y = interval(-upper_bound(y),-lower_bound(y))
    return add_interval(x, sb_y)
