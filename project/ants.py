...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        while not plc.is_hive:
            if len(plc.bees) and (distance >= self.lower_bound) and (distance <= self.upper_bound):
                return random_bee(plc.bees)  # REPLACE THIS LINE
            plc = plc.entrance
            distance += 1
        return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    # turns_to_chew = 0 it will change in calling
    def __init__(self, health=1):
        """Create an Insect with a HEALTH quantity."""
        super().__init__(health)
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        protected_ant = self.ant_contained
        if protected_ant is not None:
