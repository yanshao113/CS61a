...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        if isinstance(func, MuProcedure):
            return scheme_eval(complete_apply((func, rest, env), env, True))
        funcs = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(func, funcs, env)
