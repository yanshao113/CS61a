...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    return 2 * t_o(opponent_score) + 1


def t_o(opponent_score):
    if opponent_score <= 9:
        return opponent_score
    elif 10 <= opponent_score <= 99:
        opponent_score_t = opponent_score // 10
        opponent_score_o = opponent_score % 10
        return abs(opponent_score_t - opponent_score_o)
    else:
        opponent_score_o = opponent_score % 10
        flr = opponent_score // 10
        opponent_score_t = flr % 10
        return abs(opponent_score_t - opponent_score_o)
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        ...
        score0 = update(number0, score0, score1, dice)
        if score0 >= goal:
            return score0, score1
        number1 = strategy1(score1, score0)
        score1 = update(number1, score1, score0, dice)
        if score1 >= goal:
            return score0, score1
    # END PROBLEM 5
    return score0, score1
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def strategy(current_player_score, opponent_player_score):
        return n
    return strategy
    # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for num_rolls in range(1, 11):
        nw_av = make_averaged(roll_dice, total_samples)(num_rolls, dice)
        if nw_av > max_av:
            max_av = nw_av
            max_av_num = num_rolls
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    new_score = square_update(num_rolls=0, player_score=score, opponent_score=opponent_score)
